...
cat( "after initial cleaning: words remains" ,dim(Btmp), '\n')   
...
B[idx1,] <- B[idx1,]+B[idx2,]
terms <- terms[-idx1];  B<- B[terms,]; #update terms, Btmp
cat( "after initial cleaning + remov 's', 'ed': words remains -- "  ,dim(B), '\n')     ## 5237,635
B = (B>0)+0
rownames(B)  = terms
kw = B
...
####################################################################
####### Regularized spectral clustering  with nclust = 11 ######
####################################################################
nr = nrow(A); nc = ncol(A)
...
#write.csv(paper_core4, file = '../Data/citePaper_interpret_Core4.csv',)
